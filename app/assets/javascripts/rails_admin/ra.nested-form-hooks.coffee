...
$(document).on 'nested:fieldAdded', 'form', (content) ->
  ...
  field = content.field.addClass('tab-pane').attr('id', 'unique-id-' + uniqueId)
  ...
  field.find('.globalize_tabs_type .tab-content .localized-panel').addClass 'localized-panel-' + uniqueId
  field.find('.globalize_tabs_type .nav-tabs a').attr 'data-target', (n, v) ->
    v + '.localized-panel-' + uniqueId
